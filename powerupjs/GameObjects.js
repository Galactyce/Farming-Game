var powerupjs = (function (powerupjs) {
  ...
  SpriteGameObject.prototype.collidesWith = function (obj) {
    if (obj !== typeof Rectangle) {
      ...
      var local = intersect.position.subtractBy(
        this.worldPosition.subtract(this.origin)
      );
      var objLocal = intersect.position.subtractBy(
        obj.worldPosition.subtract(obj.origin)
      );
    } else {
      if (!this.visible || !this.boundingBox.intersects(obj)) return false;
    ...
    for (var x = 0; x < intersect.width; x++)
      for (var y = 0; y < intersect.height; y++) {
        if (
          this.getAlpha(Math.floor(local.x + x), Math.floor(local.y + y)) !==
            0 &&
          obj.getAlpha(
            Math.floor(objLocal.x + x),
            Math.floor(objLocal.y + y)
          ) !== 0
        )
          return true;
      }
  ...
  GameObjectList.prototype.remove = function (gameobject) {
    for (var i = 0, l = this.gameObjects.length; i < l; ++i) {
      if (gameobject !== this.gameObjects[i]) continue;
  ...
  GameObjectList.prototype.find = function (id) {
    for (var i = 0, l = this.gameObjects.length; i < l; ++i) {
      ...
      if (
        ...
        this.gameObjects[i] instanceof powerupjs.GameObjectGrid
      ) {
  ...
  GameObjectList.prototype.at = function (index) {
    if (index < 0 || index >= this.gameObjects.length) return null;
  ...
  GameObjectGrid.prototype.update = function (delta) {
    for (var i = 0; i < this.gridLength; i++) {
      if (this.atIndex(i) !== null && this.atIndex(i) !== undefined)
        this.gameObjects[i].update(delta);
    }
  };
  ...
  GameObjectGrid.prototype.add = function (gameobject) {
    ...
    gameobject.parent = this;
    gameobject.position = new powerupjs.Vector2(
      col * this.cellWidth,
      row * this.cellHeight
    );
  };
  ...
  GameObjectGrid.prototype.addAt = function (gameobject, col, row, position) {
    var position =
      typeof position !== "undefined" ? position : new powerupjs.Vector2(0, 0);
    this.gameObjects[row * this.columns + col] = gameobject;
    gameobject.parent = this;
    gameobject.position = new powerupjs.Vector2(
      col * this.cellWidth + position.x,
      row * this.cellHeight + position.y
    );
  };
  ...
  GameObjectGrid.prototype.getTileType = function (x, y) {
    if (x < 0 || x > this.columns) return TileType.normal;
    if (y < 0 || y > this.rows - 1) return TileType.background;
  ...
  AnimatedGameObject.prototype = Object.create(
    powerupjs.SpriteGameObject.prototype
  );

  AnimatedGameObject.prototype.loadAnimation = function (
    animationName,
    id,
    looping,
    frameTime
  ) {
    this.animations[id] = new powerupjs.Animation(
      animationName,
      looping,
      frameTime
    );
